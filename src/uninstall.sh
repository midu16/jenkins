...
# Defining different causes to be used based on the type of the OS
 ...
 if cat /etc/*release | grep ^NAME | grep CentOS; then
    ...
    yum purge -y $YUM_SYSTEM_UPDATE
    #Perform the java installation
    yum purge -y $YUM_PACKAGE_NAME
    #Perform the jenkins installation
    $YUM_JENKINS_KEY
    $YUM_JENKINS_KEY_IMPORT
    yum purge -y $YUM_JENKINS_INSTALL
    ...
    apt-get update
    #Perform the java installation
    apt-get purge -y $DEB_PACKAGE_NAME
    #Perform the jenkins installation
    $DEB_JENKINS_KEY
    sudo rm -rf /etc/apt/sources.list.d/jenkins.list
    apt-get purge -y $DEB_JENKINS_INSTALL
    ...
    echo "OS NOT DETECTED, couldn't remove package $PACKAGE"
    exit 1;
 fi
